<div>
    <div class="w-full relative">
        <input class="w-full disabled:opacity-50" placeholder="{{__('Chercher parmi les visiteurs enregistrés')}}" type="text" wire:model="searchQuery" wire:keyup="searchVisitor" @if ( $visitorSet ) disabled @endif>
        @if ( $visitorSet )
    ...
    @if ( $displayAddVisitorButton )
        <li class="p-2 border-2 cursor-pointer"><a href="#" wire:click.prevent="$emit('newVisitorForm')">
            ...
            <span>{{ __("Créer un nouveau visiteur") }}</span>
        </a></li>
    @endif
